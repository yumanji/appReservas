...
class Calendar extends Controller {
	...
	function index()
	{
		...
		$calendario = $this->load->view('calendar/calendar', array(), true);
		...
		$data=array(
			'meta' => $this->load->view('meta', array('lib_calendar2' => TRUE), true),
...
# -------------------------------------------------------------------
	...
	{
		$this->load->library('calendario_usuario');
		...
		switch ($method) {
		    case "add":
		        $ret = $this->calendario_usuario->addCalendar($this->input->post("CalendarStartTime"), $this->input->post("CalendarEndTime"), $this->input->post("CalendarTitle"), $this->input->post("IsAllDayEvent"));
		        break;
		    case "list":
						$this->load->model('Reservas_model', 'reservas', TRUE);
						$this->load->model('Retos_model', 'retos', TRUE);
		        $ret = $this->calendario_usuario->listCalendar($this->input->post('showdate'), $this->input->post('viewtype'));
		        //$ret = $this->calendario->listCalendar('11/3/2010', 'week');
		        break;
		    case "update":
		        $ret = $this->calendario_usuario->updateCalendar($this->input->post("calendarId"), $this->input->post("CalendarStartTime"), $this->input->post("CalendarEndTime"));
		        break; 
		    case "remove":
		        $ret = $this->calendario_usuario->removeCalendar( $this->input->post("calendarId"));
		        break;
		    case "adddetails":
		        ...
		        if(isset($id)){
		            ...
		            $ret = $this->calendario_usuario->updateDetailedCalendar($id, $st, $et, 
		        	...
		        	$all_day = $this->input->post("IsAllDayEvent");
		            $ret = $this->calendario_usuario->addDetailedCalendar($st, $et,                    
		                $this->input->post("Subject"), isset($all_day)?1:0, $this->input->post("Description"), 
		                $this->input->post("Location"), $this->input->post("colorvalue"), $this->input->post("timezone"));
		        }        
		        break; 
