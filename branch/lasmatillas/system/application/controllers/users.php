...
# -------------------------------------------------------------------
	function pay_quota($code_user)
	{
		...
		{			
			...
			$pagado = $this->users_lib->pay_user_quota($code_user, array('payable_quota' => $payable_quota, 'status' => $estado, 'paymentway' => $paymentway, 'code_price' => $array_result['code_price'], 'name' => trim($array_result['user_name'].' '.$array_result['user_lastname']), 'group_id' => $array_result['group_id']));
...
# -------------------------------------------------------------------
	function create_user()
	{
		...
		{			
			...
			$id_user = $this->redux_auth->register($this->input->post('password_user'), $this->input->post('email'), FALSE);
			if ($id_user)
			{
