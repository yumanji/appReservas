...
public function jqgrid_list_all_public ()
	{
      ...
      exit(0);
		...
		if($where!='') $where.=' AND ';
		$where.= 'booking.shared = \'1\' AND (booking.date >= \''.date($this->config->item('date_db_format')).'\' OR booking.id_transaction IN (SELECT DISTINCT booking.id_transaction FROM booking, booking_players WHERE  booking.shared = \'1\' AND booking.id_transaction = booking_players.id_transaction AND booking_players.id_user = \''.$user_id.'\'))';
