...
*/
    ...
    function updateTimetable($fecha, $pista, $disponibilidad) {
    	...
    	foreach ($handle->result() as $row) {
    		...
    		while (($hora + ($reserve_interval * 60)) <= $final) {
    			//array_push($cursos, $pista.'-'.$hora);
    			$cursos[$pista.'-'.$hora] = array('id' => $row->id, 'horario' => date($this->config->item('reserve_hour_filter_format'), strtotime($fecha." ".$row->start_time)).' - '.date($this->config->item('reserve_hour_filter_format'), strtotime($fecha." ".$row->end_time)));
    			$hora = $hora + ($reserve_interval * 60);
    		}
			...
			foreach($disponibilidad as $intervalo => $valores) {
				if($valores[1]=="1" && isset($cursos[$intervalo])) {
					$disponibilidad[$intervalo][1]="0";
					$disponibilidad[$intervalo][2]=$cursos[$intervalo]['id'];
					$disponibilidad[$intervalo][3]="l";
					$disponibilidad[$intervalo][4]=$cursos[$intervalo]['horario'];
				}
			}
...
##############################################################################
    ...
    function getLevelsArray()
    {
        ...
        $sql = "SELECT id, description FROM zz_lessons_levels ORDER BY Description"; 
...
##############################################################################
	...
	function get_AssitantsData($params = "" , $page = "all")
  {
  	$this->load->library('app_common');
        ...
        //return $query->result();
        $result=array();
    $this->db->select('lessons_assistants.id, lessons_assistants.id_lesson, lessons_assistants.status, lessons_assistants.id_user, lessons_assistants.user_desc, lessons_assistants.user_phone, lessons_assistants.unsubscription_date, lessons_assistants.sign_date, lessons_assistants.last_payd_date, lessons_assistants.last_day_payed, concat( lessons_assistants.discount , lessons_assistants.discount_type ) as discount, lessons_assistants.discount as discount_value, lessons_assistants.discount_type as discount_type, meta.first_name, meta.last_name, meta.phone, lessons.description, lessons.weekday, lessons.L, lessons.M, lessons.X, lessons.J, lessons.V, lessons.S, lessons.D, lessons.start_time, lessons.end_time, lessons.start_date, lessons.end_date, lessons.max_vacancies, lessons.current_vacancies, lessons.id_sport as sport, lessons.price as price, zz_sports.description as sport_desc, users.group_id as group_id');
...
##############################################################################
    ...
    function addAssistant($lesson, $status, $user = NULL, $user_desc = NULL, $user_phone = NULL, $discount = 0)
    {
    	try {
        //$query = $this->db->get('entries', 10);
        //return $query->result();
...
##############################################################################
    ...
    function unsubscribeAssitant($lesson, $id)
    {
    	try {
        //$query = $this->db->get('entries', 10);
        //return $query->result();
