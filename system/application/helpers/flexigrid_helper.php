...
if (! function_exists('build_grid_js'))
{
	 ...
	 * @return	string
	 */
	function build_grid_js($grid_id,$url,$colModel,$sortname,$sortorder,$gridParams = NULL,$buttons = NULL, $extra_where = NULL)
	{
		//Basic propreties
		...
		if (is_array($buttons)) 
		{
			...
			foreach ($buttons as $index => $value) {
				if ($value[0] == 'separator')
					...
					$grid_js .= "{name: '".$value[0]."', bclass : '".$value[1]."', onpress : ".$value[2].", process: ".$value[2]."},";
			}
			//Remove the last ","
			$grid_js = substr($grid_js,0,-1).']';
		} 
