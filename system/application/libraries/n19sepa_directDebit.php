 ...
 *
 */
class n19sepa_DirectDebit extends n19sepa_Base {
    ...
    protected function _generateXml() {
		...
		//print_r($this->_transactions);
		//exit('aaa'.$nrOfTransactions);
        /**
         * Document base
         */
    ...
    public function addTransaction($Transaction) {
        if (!$Transaction instanceof Sepa_DirectDebit_Transaction)
            throw new n19sepa_Exception('Invalid transaction');
    ...
    public function setCreditorIban($creditorIban) {
        if ( !$creditorIban || strlen($creditorIban) > 34 ) throw new n19sepa_Exception('Invalid IBAN value. Accepted: min-length: 1, max-length: 32');
    ...
    public function setCreditorBic($creditorBic) {
        if ( !preg_match('/^[0-9a-z]{4}[a-z]{2}[0-9a-z]{2}([0-9a-z]{3})?\z/i', $creditorBic ) ) throw new n19sepa_Exception('Invalid BIC. Accepted: regex: /^[0-9a-z]{4}[a-z]{2}[0-9a-z]{2}([0-9a-z]{3})?\z/i');
    ...
    public function setPaymentInfoId($paymentInfoId) {
        if ( !$paymentInfoId || strlen($paymentInfoId) > 35 ) throw new n19sepa_Exception('Invalid value for PaymentInfoId. Accepted: min-length: 1, max-length: 32');
    ...
    public function setRequestedExecutionDate($requestedExecutionDate) {
        ...
        if ( !checkdate($dateArr[1], $dateArr[2], $dateArr[0]) ) throw new n19sepa_Exception('Invalid date for RequestedExecutionDate. Accepted format: yyyy-mm-dd');
