...
function jqGrid_ondblClickRow() {
	...
	if(gsr){
		...
		location.href='<?php echo site_url('users/detail'); ?>/'+gsr;
...
$(function() {
		...
		jQuery("#grid_name").jqGrid('navButtonAdd','#pager2',{caption:"", buttonicon:"ui-icon-person", title:'Ver detalle',
		...
		jQuery("#grid_name").jqGrid('navButtonAdd','#pager2',{caption:"", buttonicon:"ui-icon-plusthick", title:'Nuevo usuario',
			onClickButton:function(){
			...
			if(isset($permisos['export_excel']) && $permisos['export_excel']) {
				$parame = $this->uri->segment(3);
				if(!isset($parame) || $parame== '') $parame = 'all';
		?>
			jQuery("#grid_name").jqGrid('navButtonAdd','#pager2',{caption:"", buttonicon:"ui-icon-extlink", title:'Exportar a excel',
				onClickButton:function(){
						...
						document.getElementById('hora_fin_view').value = horaFin;
						*/
						location.href='<?php echo site_url('users/index/'.$parame.'/excel'); ?>';
