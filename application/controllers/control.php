...
####################
	...
	function lessons_quotas_torrijos_list($curso_id = NULL, $curso2_id = NULL)
	{
	exit();
		$this->load->model('redux_auth_model', 'users', TRUE);
		$this->load->helper('jqgrid');
		...
		$datos = $this->lessons->get_data(array('where'=> "lessons.id_sport = 11 and lessons.active = 1" ), "all");
		echo "\r\n"."<ul>";
		foreach($datos as $curso) {
			echo "\r\n".'<li>Curso '.$curso['description'].' -> <a href="lessons_quotas_torrijos/'.$curso['id'].'" target="_blank">Lanzar</a></li>';
		}
		echo "\r\n"."</ul>";
		print_r($datos);
	}
...
####################
	...
	function lessons_quotas_torrijos($curso_id = NULL, $curso2_id = NULL)
	{
		...
		//echo '----'.$this->config->item('log_date_format').'---'.date($this->config->item('log_date_format'));
		#Recupero lista de usuarios
		//$add_where = " and lessons.id_sport = 11 ";
		$add_where = "";
		if(isset($curso_id)) {
			if(isset($curso2_id)) $add_where .= " and lessons.id >= '".$curso_id."' and lessons.id <= '".$curso2_id."'  ";
			else $add_where .= " and lessons.id = '".$curso_id."'  ";
		}
		...
		foreach($pagadores as $pagador) {
			...
			if($debug) echo '<br>Alumno '.$pagador['user_desc'].' del curso '.$pagador['description'];
				...
				if($pagador['group_id']>=7) {
					...
					$this->users_lib->pay_user_quota($pagador['id_user'], array('payable_quota'=> $cuota_socio, 'payd_date_tmp' => '10/01/2015'));
