...
$(function() {
		...
		jQuery("#grid_name").jqGrid('navButtonAdd','#pager2',{caption:"", buttonicon:"ui-icon-check", title:'Apuntar jugador al reto',
			onClickButton:function(){
				var gsr = jQuery("#grid_name").jqGrid('getGridParam','selrow');
				if(gsr){
					location.href='<?php echo site_url('retos/validate_player/'.$id_transaction); ?>/'+gsr;
				} else {
					alert("Please select Row")
				}							
			} 
