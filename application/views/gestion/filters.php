...
if(isset($search_fields) && count($search_fields)) {	
	echo '<fieldset';
	if(isset($fieldset_width) && $fieldset_width!="") echo ' style=" width: '.$fieldset_width.'px;" ';
	echo '>';
	...
	foreach($search_fields as $field) {
		switch($field['type']) {
			...
			break;
			case 'text':
				$extra=array('name' => $field['name'], 'value' => $field['value']);
				if(isset($field['onchange']) && $field['onchange']!="") $extra['onChange']=$field['onchange'];
				if(isset($field['id']) && $field['id']!="") $extra['id']=$field['id'];
				if(isset($field['enabled']) && !$field['enabled']) { $extra['disable']='TRUE'; }
				echo '<td><label for="'.$field['name'].'">'.$field['desc'].':</label> '.form_input($extra)."</td>\r\n";
			break;
			case 'date':
				$data=array();
				$jquery_extra=' ';
